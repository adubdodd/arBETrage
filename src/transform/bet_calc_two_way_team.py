...
group_by_columns = ["Game_ID", "League_Key", "League_Title", "Matchup", "Start_Time","Home_Team", "Away_Team"]

def implied_probability(odds): 
    return 1 / odds

def calc_probs(df: pd.DataFrame):
    try:
        ...
        best_home = df.loc[df.groupby("Game_ID")["Home_Odds"].idxmax(), group_by_columns + ["Home_Odds", "Bookmaker"]]
        best_away = df.loc[df.groupby("Game_ID")["Away_Odds"].idxmax(), group_by_columns + ["Away_Odds", "Bookmaker"]]
        ...
        print(f"[ERROR] calc_odds failed: {e}")
        raise e
